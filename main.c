...
// Module unique ID now stored directly in sg_sFrame.m.moduleUniqueId
...
//	MBASSERT(u8CellCountExpected <= (sizeof(sg_sFrame.m.cellData) >> BYTES_PER_CELL_SHIFT));
	...
	sg_sFrame.m.sg_i16VoltageStringPerADC = (int16_t)(((sg_sFrame.m.sg_i32VoltageStringMax - sg_sFrame.m.sg_i32VoltageStringMin) * ADC_VOLT_FRACTION) / ((int32_t) (1 << ADC_BITS)));  // in millivolts
}
...
static void CellStringPowerStateMachine(void)  // this is called at the start of each READ and WRITE frame
{
	switch (sg_eStringPowerState)
	{
		...
		case ESTRING_OFF: // turns power OFF, then sets the power timer to wait for the circuits to settle
		{
			...
			sg_sFrame.m.sg_u8CellCPUCountFewest = 0xff;  // reset min max cell counts
			sg_sFrame.m.sg_u8CellCPUCountMost = 0x0;
			break;
		}
		case ESTRING_ON:
		{
...
// 	CANGIE = savedCANGIE;
	...
	if( ECANMessageType_ModuleAnnounceRequest == eType )
	{
		...
		if (!sg_bModuleRegistered && !sg_bAnnouncementPending)
		{
			...
			uint8_t u8RandomDelay = (uint8_t)(sg_sFrame.m.moduleUniqueId & 0xFF);
			...
			(u8RegID == sg_u8ModuleRegistrationID))
		{
			...
			if( ECANMessageType_ModuleCellDetailRequest == eType )
			{
				if( 3 == u8DataLen )
				{
					...
					if( (false == sg_bSendCellStatus) && (pu8Data[1] < sg_sFrame.m.sg_u8CellCountExpected) )
					{
						...
						if (CELL_DETAIL_ALL == pu8Data[1])
						{
							...
							sg_u8CellStatusTarget = sg_sFrame.m.sg_u8CellCountExpected;
							sg_u8CellStatus = 0;
						}
...
//	cli();
	s16Offset = sg_sFrame.m.ADCReadings[ EADCTYPE_CURRENT0 ].u16Reading; 
	s16ZeroValue = sg_sFrame.m.ADCReadings[ EADCTYPE_CURRENT1 ].u16Reading;
//	sei();
	...
	sg_sFrame.m.u16frameCurrent = (uint16_t) iCurrent;
	if (sg_sFrame.m.u16frameCurrent > sg_sFrame.m.u16maxCurrent)
	{
		sg_sFrame.m.u16maxCurrent = sg_sFrame.m.u16frameCurrent;
	}
	if (sg_sFrame.m.u16frameCurrent < sg_sFrame.m.u16minCurrent)
	{
		sg_sFrame.m.u16minCurrent = sg_sFrame.m.u16frameCurrent;
	}
...
static void ControllerStatusMessagesSend(uint8_t *pu8Response)
{
	...
	if( sg_bSendModuleControllerStatus )
	{
		...
		if (0 == sg_u8ControllerStatusMsgCount)
		{
			...
			pu8Response[3] = sg_sFrame.m.sg_u8CellCountExpected;
				
			// Clear the rest
			memset((void *) &pu8Response[4], 0, CAN_STATUS_RESPONSE_SIZE - 4);
				
			// 16 Bit module current (-655.36A to 655.24A in 0.02A increments)
			if( sg_sFrame.m.ADCReadings[ EADCTYPE_CURRENT0 ].bValid &&
				sg_sFrame.m.ADCReadings[ EADCTYPE_CURRENT1 ].bValid &&
				(EMODSTATE_ON == sg_eModuleControllerStateCurrent))
			{
				...
				u16Temp = (uint16_t) sg_sFrame.m.u16frameCurrent;
			}
			else
			{
			...
			// go with that reading since the string voltage has a floor.
			{
				...
				if( sg_sFrame.m.ADCReadings[ EADCTYPE_STRING ].bValid )
				{
				// Analog input is inverted
				u32Voltage = (uint32_t)((1 << ADC_BITS) - 1) - sg_sFrame.m.ADCReadings[ EADCTYPE_STRING ].u16Reading;
				}
...
//				if ((sg_sFrame.m.sg_u8CellCPUCount == sg_sFrame.m.sg_u8CellCountExpected) || (0 == u32Voltage))
if(0)
				{
					...
					u32Voltage = (sg_sFrame.m.sg_u32CellVoltageTotal / 15);
				}
				else
				{
					...
					u32Voltage *= sg_sFrame.m.sg_i16VoltageStringPerADC;  // in fractions of millivolts
					u32Voltage /= ADC_VOLT_FRACTION;  // convert to millivolts 
					u32Voltage += sg_sFrame.m.sg_i32VoltageStringMin;  // in millivolts
					
					// Convert whatever final voltage we have to 0.015 volt increments
					u32Voltage /= 15;
				}

				sg_sFrame.m.sg_i32VoltageStringTotal = (int32_t) u32Voltage;
		...
		if (1 == sg_u8ControllerStatusMsgCount)
		{
			...
			pu8Response[0] = (uint8_t) sg_sFrame.m.sg_u16LowestCellVoltage;
			pu8Response[1] = (uint8_t) (sg_sFrame.m.sg_u16LowestCellVoltage >> 8);

			// 16 Bit highest cell voltage (0.65.535 volts in millivolt increments)
			pu8Response[2] = (uint8_t) sg_sFrame.m.sg_u16HighestCellVoltage;
			pu8Response[3] = (uint8_t) (sg_sFrame.m.sg_u16HighestCellVoltage >> 8);
				
			// 16 Bit average cell voltage (0.65.535 volts in millivolt increments)
			pu8Response[4] = (uint8_t) sg_sFrame.m.sg_u16AverageCellVoltage;
			pu8Response[5] = (uint8_t) (sg_sFrame.m.sg_u16AverageCellVoltage >> 8);
				
			// 16 Bit total cell voltage in 0.015v increments
			u16Voltage = (uint16_t) (sg_sFrame.m.sg_u32CellVoltageTotal / 15);  //cell voltage total is in mv
		...
		if (2 == sg_u8ControllerStatusMsgCount)
		{
			...
			pu8Response[0] = (uint8_t) sg_sFrame.m.sg_s16LowestCellTemp;
			pu8Response[1] = (uint8_t) (sg_sFrame.m.sg_s16LowestCellTemp >> 8);

			// 16 Bit highest cell temperature (-55.35 base + 0.01 degree C increments)
			pu8Response[2] = (uint8_t) sg_sFrame.m.sg_s16HighestCellTemp;
			pu8Response[3] = (uint8_t) (sg_sFrame.m.sg_s16HighestCellTemp >> 8);
				
			// 16 Bit average cell temperature (-55.35 base + 0.01 degree C increments)
			pu8Response[4] = (uint8_t) sg_sFrame.m.sg_s16AverageCellTemp;
			pu8Response[5] = (uint8_t) (sg_sFrame.m.sg_s16AverageCellTemp >> 8);
	...
	if( sg_bSendCellStatus )
	{
		...
		if ((sg_sFrame.m.sg_u16HighestCellVoltage > sg_sFrame.m.sg_u16LowestCellVoltage) && (u16Voltage > 0))
		{
			pu8Response[7] = ((uint32_t) u16Voltage / (uint32_t) (sg_sFrame.m.sg_u16HighestCellVoltage - sg_sFrame.m.sg_u16LowestCellVoltage) * (uint32_t) 100.0);
		}
		else
		{
			pu8Response[7] = 0;
	...
	if( sg_bSendCellCommStatus )
	{
		...
		pu8Response[0] = sg_sFrame.m.sg_u8CellCPUCountFewest;
		pu8Response[1] = sg_sFrame.m.sg_u8CellCPUCountMost;
				
		// I2C Faults
		pu8Response[2] = (uint8_t) sg_sFrame.m.sg_u16CellCPUI2CErrors;
		pu8Response[3] = (uint8_t) (sg_sFrame.m.sg_u16CellCPUI2CErrors >> 8);
				
		// MC RX framing errors
		pu8Response[4] = sg_sFrame.m.sg_u8MCRXFramingErrors;

		// Which first cell # is generating an I2C error?
		
		if (sg_sFrame.m.sg_u8CellFirstI2CError != 0xff)
		{
			// Need to invert it since the oldest is at the start of the list
			pu8Response[5] = (sg_sFrame.m.sg_u8CellCPUCount - sg_sFrame.m.sg_u8CellFirstI2CError); // - 1;
		}
		else
		{
			// None are in error
	...
	if (sg_bSendHardwareDetail)
	{
		...
		// Encoded in 0.015V (15mV) increments to match module voltage reporting
		{
			...
			u32EndOfChargeVoltage = (uint32_t)sg_sFrame.m.sg_u8CellCountExpected * CHEMISTRY_VOLTAGE_MV;
...
static void CellStringProcess(uint8_t *pu8Response)  // no longer does float calcs on every cell, doesn't do anything with pu8Response
{
	...
	if (sg_sFrame.m.sg_u16BytesReceived)
	{
		// Assume there are no I2C errors...
		sg_sFrame.m.sg_u8CellFirstI2CError = 0xff;

		// Update comm stats
		if (sg_sFrame.m.sg_u8CellCPUCountFewest > sg_sFrame.m.sg_u8CellCPUCount)
		{
			sg_sFrame.m.sg_u8CellCPUCountFewest = sg_sFrame.m.sg_u8CellCPUCount;
			sg_bSendCellCommStatus = true;
		}
		if (sg_sFrame.m.sg_u8CellCPUCountMost < sg_sFrame.m.sg_u8CellCPUCount)
		{
			sg_sFrame.m.sg_u8CellCPUCountMost = sg_sFrame.m.sg_u8CellCPUCount;
			sg_bSendCellCommStatus = true;
		}
		...
		{
			if (sg_sFrame.m.sg_u8MCRXFramingErrors != 0xff)
			{
				sg_sFrame.m.sg_u8MCRXFramingErrors++;
				sg_bSendCellCommStatus = true;
			}
		}

		// Find the highest and lowest temperatures and voltages for this batch of cell data
		sg_sFrame.m.sg_u16HighestCellVoltage = 0;
		sg_sFrame.m.sg_u16LowestCellVoltage  = 0xffff;
		sg_sFrame.m.sg_u16AverageCellVoltage = 0;
		sg_sFrame.m.sg_s16HighestCellTemp = -32768;
		sg_sFrame.m.sg_s16LowestCellTemp = 32767;
		sg_sFrame.m.sg_s16AverageCellTemp = 0;
		...
		{
			sg_sFrame.m.sg_u32CellVoltageTotal = u32CellVoltageTotalmV;
			sg_sFrame.m.sg_u16AverageCellVoltage = (uint16_t)(sg_sFrame.m.sg_u32CellVoltageTotal / u8CellVoltageCount);

			if ((EMODSTATE_ON != sg_eModuleControllerStateCurrent) &&
			(false == sg_bCellBalancedOnce) &&
			(sg_sFrame.m.sg_u16HighestCellVoltage >= sg_sFrame.m.sg_u16LowestCellVoltage) &&
			((sg_sFrame.m.sg_u16HighestCellVoltage - sg_sFrame.m.sg_u16LowestCellVoltage) >= BALANCE_VOLTAGE_THRESHOLD))
			{
				sg_bCellBalanceReady = true;
			}
		}
		...
		if (u8CellTemperatureCount)  // got at least some
		{
			...
			CellDataConvertTemperature(s16AverageTempRAW,&sg_sFrame.m.sg_s16AverageCellTemp);
			CellDataConvertTemperature(s16HighestCellTempRAW,&sg_sFrame.m.sg_s16HighestCellTemp);
			CellDataConvertTemperature(s16LowestCellTempRAW,&sg_sFrame.m.sg_s16LowestCellTemp);
		}

	}  // end if (sg_sFrame.m.sg_u16BytesReceived)
	else
	{
...
void FrameInit(bool  bFullInit)  // receives true if full init is needed, false if only cell reading data is to be reset
{
		...
		if (bFullInit || (FRAME_VALID_SIG != sg_sFrame.m.validSig)) // full init or invalid frame
		{
		...
		else  // do only if partial init, in full init the memset takes care of all this
		{
			...
			sg_sFrame.m.sg_u8CellCPUCount = 0;     // for the frame
			sg_sFrame.m.sg_u8MCRXFramingErrors = 0;
			
			// Only reset voltage/temp stats if we have no valid data
			// This prevents the alternating zero issue when status is polled
			if (sg_sFrame.m.sg_u32CellVoltageTotal == 0)
			{
				sg_sFrame.m.sg_u32CellVoltageTotal = 0;		// All cell CPU voltages added up (in millivolts)
				sg_sFrame.m.sg_u16HighestCellVoltage = 0;
				sg_sFrame.m.sg_u16LowestCellVoltage = 0;
				sg_sFrame.m.sg_u16AverageCellVoltage = 0;
				sg_sFrame.m.sg_s16HighestCellTemp = TEMPERATURE_BASE;	// 0C default, in case we get no readings
				sg_sFrame.m.sg_s16LowestCellTemp = TEMPERATURE_BASE;		// 0C default, in case we get no readings
				sg_sFrame.m.sg_s16AverageCellTemp = TEMPERATURE_BASE;	// 0C default, in case we get no readings
			}
		}
...
// 				CANGIE = savedCANGIE;			
			...
			if (EFRAMETYPE_WRITE == eCurrentFrame)  // only do these ops if vUART is idle!
			{
				if(bFrameStart) // start of WRITE frame, reference local variable saved earlier
				{
					...
					if (ESTRING_OPERATIONAL == sg_eStringPowerState)
					{
						...
						(sg_sFrame.m.sg_u8CellCountExpected))
						{
					...
					if( sg_bSendAnnouncement )  //we should announce ourselves
					{
						...
						u8Reply[3] = PART_ID;
						*((uint32_t*)&u8Reply[4]) = sg_sFrame.m.moduleUniqueId;
	...
	//------------------------- section specific to READ frame --------------------------------
		...
		//				sg_sFrame.m.sg_u16BytesReceived = sizeof(sg_u16FakeCellData);
						sg_sFrame.m.sg_u16BytesReceived = sg_sFrame.m.sg_u8CellCountExpected << 2;
		//				sg_sFrame.m.sg_u8CellCPUCount = sizeof(sg_u16FakeCellData) >> 2;	// Each cell report is 4 bytes
						sg_sFrame.m.sg_u8CellCPUCount = sg_sFrame.m.sg_u8CellCountExpected;
		#else  // make it
